	...
	: Super(ObjectInitializer)
{
	...
	bHasMips = false; // FB Bulgakov - auto mip generation fix
...
FTextureResource* UTextureRenderTarget2D::CreateResource()
{
	...
	if (bHasMips) // FB Bulgakov - auto mip generation fix
	{
...
void FTextureRenderTarget2DResource::InitDynamicRHI()
{
	if( TargetSizeX > 0 && TargetSizeY > 0 )
	{
		...
		if (Owner->bHasMips) // FB Bulgakov - auto mip generation fix
		{
		...
		AddToDeferredUpdateList(!(Owner->bHasMips && Owner->bAutoGenerateMips)); // FB Bulgakov - auto mip generation fix
	}
...
void FTextureRenderTarget2DResource::UpdateDeferredResource( FRHICommandListImmediate& RHICmdList, bool bClearRenderTarget/*=true*/ )
{
	...
	if (Owner->bHasMips && Owner->bAutoGenerateMips) // FB Bulgakov - auto mip generation fix
	{
