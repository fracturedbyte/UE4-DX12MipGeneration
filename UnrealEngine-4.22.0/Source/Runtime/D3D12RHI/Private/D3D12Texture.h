...
public:
	...
	FD3D12TextureBase(class FD3D12Device* InParent)
		...
		, bCreatedUAVsPerSlice(false) // FB Bulgakov - DX12 mip generation
	...
	inline void SetCreatedUAVsPerSlice(bool Value, int32 InUAVArraySize)
	{
		bCreatedUAVsPerSlice = Value;
		UAVArraySize = InUAVArraySize;
	}
	// FB End Bulgakov
	...
	// FB Begin Bulgakov - DX12 mip generation
	void SetNumUnorderedAccessViews(int32 InNumViews)
	{
		UnorderedAccessViews.Empty(InNumViews);
		UnorderedAccessViews.AddDefaulted(InNumViews);
	}
	// FB End Bulgakov
	...
	// FB Begin Bulgakov - DX12 mip generation
	void SetUnorderedAccessViewIndex(FD3D12UnorderedAccessView* View, uint32 SubResourceIndex)
	{
		if (SubResourceIndex < (uint32)UnorderedAccessViews.Num())
		{
			UnorderedAccessViews[SubResourceIndex] = View;
		}
		else
		{
			check(false);
	...
	void SetUnorderedAccessView(FD3D12UnorderedAccessView* View)
	{
		UnorderedAccessViews.Empty(1);
		UnorderedAccessViews.Add(View);
	}
	// FB End Bulgakov
	...
	*/
	...
	// FB Begin Bulgakov - DX12 mip generation
	FD3D12UnorderedAccessView* GetUnorderedAccessView(int32 MipIndex, int32 ArraySliceIndex) const
	{
		int32 ArrayIndex = MipIndex;

		if (bCreatedUAVsPerSlice)
		{
			check(ArraySliceIndex >= 0);
			ArrayIndex = MipIndex * UAVArraySize + ArraySliceIndex;
			check(ArrayIndex < UnorderedAccessViews.Num());
		}
		else
		{
			// Catch attempts to use a specific slice without having created the texture to support it
		...
		if (ArrayIndex < UnorderedAccessViews.Num())
		{
			return UnorderedAccessViews[ArrayIndex];
		}
		return 0;
	}
	// FB End Bulgakov
	...
	// FB Begin Bulgakov - DX12 mip generation
	inline bool HasUnorderedAccessViews() const
	{
		return (UnorderedAccessViews.Num() > 0);
	}
	// FB End Bulgakov
...
protected:
	...
	// FB Begin Bulgakov - DX12 mip generation
	/** A unordered access view of the texture. */
	TArray<TRefCountPtr<FD3D12UnorderedAccessView>, TInlineAllocator<1>> UnorderedAccessViews;

	bool bCreatedRTVsPerSlice;
	
	bool bCreatedUAVsPerSlice;
